...
function assertTargetPayload({ targetType, targetIdentity, productId, variantId }) {
  ...
  if (targetType === "PRODUCT") {
    ...
    const expected = buildAutomaticRuleTargetIdentity({ targetType, productId });
  ...
  if (targetType === "VARIANT") {
    ...
    const expected = buildAutomaticRuleTargetIdentity({ targetType, productId, variantId });
    if (targetIdentity !== expected) {
...
export const automaticProductRuleStateRepository = {
  ...
  async upsertState(
    { automaticProductRuleId, shop, targetType, targetIdentity, productId = null, variantId = null, data = {} },
    db = prisma,
  ) {
    assertStateOwnerPayload({ automaticProductRuleId, shop });
    assertTargetPayload({
