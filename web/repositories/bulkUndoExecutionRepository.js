...
async function updateHistoryWithUndoFence({
  ...
  if (allowedStates.length && !allowedStates.includes(currentUndo.executionState)) {
    return { count: 0 };
  }
  ...
  const updated = await prisma.editHistory.updateMany({
    ...
    data: {
      undo: {
        ...
        executionState: BULK_UNDO_STATES.DISPATCHING,
  ...
  return prisma.$transaction(
    async (tx) => {
      ...
      if (
        ...
        operation.shopifyBulkOperationId
      ) {
      ...
      await tx.outboxEvent.create({
        data: {
          ...
          eventDedupeKey: `undo-recovery:${operation.id}:${recoveryId}`,
          payloadJson: {
            shop,
      ...
      await tx.bulkEditRecoveryAudit.create({
        data: {
          ...
          metadata: {
            ...
            recoveryId,
            previousState: operation.executionState,
...
export async function reopenFalseCompletedUndoExecution({
  ...
  const recoveryReason = String(reason || "").trim();
  if (!recoveryReason) throw new Error("RECOVERY_REASON_REQUIRED");
  if (!evidence?.shopifyBulkOperationId || !evidence?.verificationFailure) {
  ...
  return prisma.$transaction(
    async (tx) => {
      ...
      if (
        !operation ||
        operation.executionState !== "completed" ||
        operation.shopifyBulkOperationId
      ) {
      ...
      if (
        ...
        undo.executionState !== BULK_UNDO_STATES.COMPLETED
      ) {
      ...
      await tx.outboxEvent.create({
        data: {
          ...
          aggregateId: operation.id,
          domainEventType: "UNDO_REQUESTED",
          eventDedupeKey: `undo-false-completion-recovery:${operation.id}:${recoveryId}`,
          payloadJson: {
            shop,
            historyId: operation.sourceEditHistoryId,
            undoExecutionId: operation.id,
...
export async function moveUndoToAwaitingShopify({
  ...
  return updateHistoryWithUndoFence({
    ...
    buildData: (currentUndo) => ({
      shopifyBulkOperationId,
      processingChunkId: `${undo.executionIdentity || historyId}:${
        cursorId || "start"
      }`,
      batch: {
        ...batch,
      ...
      undo: {
        ...currentUndo,
        outcomeStatus: "processing",
        executionState: BULK_UNDO_STATES.AWAITING_SHOPIFY,
        shopifyBulkOperationId,
