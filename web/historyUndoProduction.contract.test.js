...
test("history list selects undo evidence only from target snapshots", () => {
  ...
  assert.doesNotMatch(
    listQuery[1],
    /undoStatus:\s*true|undoMutation:\s*true|undoErrorCode:\s*true|undoErrorMessage:\s*true|undoneAt:\s*true/
  );
  assert.match(
    service,
    /db\.targetSnapshotItem\.findMany\([\s\S]*undoMutation:\s*true/
  );
});
...
test("outbox dispatcher publishes a deterministic BullMQ undo job and recovers stale claims", () => {
  ...
  assert.match(dispatcher, /event\.domainEventType === "UNDO_REQUESTED"/);
...
test("undo paging uses a ChangeRecord cursor and webhook ingestion uses scalar fencing", () => {
  ...
  assert.match(ingestion, /shopifyBulkOperationId: String\(shopifyBulkOperationId\)/);
  assert.match(ingestion, /updatedAt: history\.updatedAt/);
  assert.doesNotMatch(ingestion, /path: \["shopifyBulkOperationId"\]/);
  assert.doesNotMatch(ingestion, /path: \["state"\]/);
  assert.match(ingestion, /failureCode: null/);
  assert.match(ingestion, /failureMessage: null/);
});
