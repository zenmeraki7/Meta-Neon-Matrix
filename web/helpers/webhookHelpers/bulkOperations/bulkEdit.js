...
export async function handleProductEditOperation({ bulkOperationId, shop = null }) {
  ...
  try {
    ...
    const bulkStatus = String(bulkOperation?.status || "").toUpperCase();
    const bulkType = String(bulkOperation?.type || "").toUpperCase();
    const isCompletedMutationEdit =
      claimKind === "edit" &&
      bulkType === "MUTATION" &&
      bulkStatus === "COMPLETED";

    // Legacy webhook finalizer is disabled for completed edit mutations.
    // Completion is owned by result-ingest + verification workers.
    if (isCompletedMutationEdit) {
      return {
        success: true,
        skipped: true,
        reason: "legacy_completed_mutation_finalizer_disabled",
      };
    }

    if (["CREATED", "RUNNING", "CANCELING"].includes(bulkStatus)) {
      await setLifecycleExecutionState(history, OPERATION_LIFECYCLE_STATES.SHOPIFY_RUNNING);
      return { success: true, skipped: true, reason: "shopify_running" };
    }
    if (bulkStatus === "COMPLETED") {
      await setLifecycleExecutionState(history, OPERATION_LIFECYCLE_STATES.SHOPIFY_COMPLETED);
    }
    const hasFailure =
    ...
    await setLifecycleExecutionState(history, OPERATION_LIFECYCLE_STATES.INGESTING_RESULTS);
    const resultIngestStage = await beginEditHistoryStage({
      historyId: history.id,
      shop: history.shop,
      stage: "resultIngest",
      executionId: history.executionIdentity || history.id,
    ...
    if (resultIngestStage.state === "started") {
      try {
        const ingestionSummary = await ingestBulkOperationResultRows(history, bulkOperation);
        await prisma.editHistory.update({
          where: { id: history.id },
          data: {
            batch: {
        ...
        await failEditHistoryStage({
          ...
          stage: "resultIngest",
          retryable: true,
          checkpoint: { bulkOperationId },
          error: error.message,
        });
        throw error;
      }
    }
    await setLifecycleExecutionState(history, OPERATION_LIFECYCLE_STATES.MIRROR_UPDATING);
