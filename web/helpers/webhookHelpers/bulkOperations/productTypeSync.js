...
async function transitionMirrorSyncFromHistory({
  ...
  const updated = await prisma.operationFingerprint.updateMany({
    where: {
      shop,
...
export async function handleSyncOperation({ bulkOperationId, shop = null }) {
  ...
  try {
    ...
    if (syncHistory.operationType === "Product" && !syncHistory.syncBatchId) {
      ...
      const syncBatchSet = await prisma.syncHistory.updateMany({
        where: {
          id: syncHistory.id,
    ...
    if (syncHistory.operationType === "Collection") {
      ...
      const completed = await prisma.syncHistory.updateMany({
        where: {
          id: syncHistory.id,
          shop: syncHistory.shop,
          status: { in: ACTIVE_SYNC_STATUSES },
