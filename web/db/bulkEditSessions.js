 ...
 * @param {{ shop: string, rawFilterInput?: object, variantCount?: number }} input
 * @returns {Promise<object>}
 */
export async function createSession({ shop, rawFilterInput = {}, variantCount = 0 }) {
  ...
  const rows = await prisma.$queryRaw`
    INSERT INTO bulk_edit_sessions (
      ...
      ${resolvedShop},
      'DRAFT',
      ${JSON.stringify(rawFilterInput || {})}::jsonb,
...
/**
 * Commits a session scoped to shop.
 * @param {string} sessionId
 * @param {string} shop
 * @returns {Promise<object|null>}
 */
export async function commitSession(sessionId, shop) {
  const resolvedId = String(sessionId || "").trim();
  const resolvedShop = String(shop || "").trim();
  if (!resolvedId || !resolvedShop) {
    throw new Error("commitSession requires sessionId and shop");
  }
...
/**
 * Updates a session status by id.
 * @param {string} sessionId
 * @param {string} status
 * @returns {Promise<object|null>}
 */
