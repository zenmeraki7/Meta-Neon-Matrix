...
test("Product state identity is PRODUCT:<productId> and variantId is null", () => {
  const rule = { targetResourceType: "PRODUCT" };
  ...
  assert.equal(targets[0].targetResourceType, "PRODUCT");
...
test("Variant state does not collapse across sibling variants", () => {
  const rule = { targetResourceType: "VARIANT" };
  const product = {
...
test("Backfill identity for old product-only rows remains PRODUCT:<productId>", () => {
  ...
  assert.equal(targetResourceType, "PRODUCT");
...
test("Target identity helper and shape validation enforce PRODUCT/VARIANT invariants", () => {
  ...
  assert.equal(
    buildAutomaticRuleTargetIdentity({ targetResourceType: "PRODUCT", productId }),
    "PRODUCT:gid://shopify/Product/1",
  );
  assert.equal(
    buildAutomaticRuleTargetIdentity({ targetResourceType: "VARIANT", productId, variantId }),
    "VARIANT:gid://shopify/ProductVariant/2",
  );
  assert.throws(
    () => buildAutomaticRuleTargetIdentity({ targetResourceType: "VARIANT", variantId }),
    /VARIANT rule target requires productId and variantId/,
  );

  assert.doesNotThrow(() =>
    assertAutomaticRuleTargetShape({
      rule: { targetResourceType: "PRODUCT" },
      target: { productId, variantId: null },
    }),
  );
  assert.doesNotThrow(() =>
    assertAutomaticRuleTargetShape({
      rule: { targetResourceType: "VARIANT" },
