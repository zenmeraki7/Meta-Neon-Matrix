...
function getOneTimeNextRunAt({ scheduleConfig, timezone }, fromDate) {
  const availableAt = scheduleConfig?.availableAt;
  if (!availableAt) {
    throw new Error("One-time schedules require availableAt");
  }

  const candidate = toTimezoneMoment(availableAt, timezone);
