...
function normalizeFilterParams(value) {
  ...
  if (!Array.isArray(value)) {
    throw buildCommandError(
      ...
      "rawFilterInput must be an array",
    );
  }
  ...
  return value.slice(0, MAX_FILTER_PARAMS).map((filter, index) => {
    if (!isPlainObject(filter)) {
      throw buildCommandError(
        ...
        "Each rawFilterInput item must be an object",
        { index },
      );
    }
...
function normalizeRecurringEditBody(body, { partial = false } = {}) {
  ...
  const normalized = {
    ...
    targetDefinitionHash:
      safeBody.targetDefinitionHash === undefined
        ? null
        : safeString(safeBody.targetDefinitionHash, null, MAX_ID_LENGTH),
    previewContractId:
    ...
    rawFilterInput:
      safeBody.rawFilterInput === undefined
        ? null
        : normalizeFilterParams(safeBody.rawFilterInput),
