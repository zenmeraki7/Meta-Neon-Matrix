  ...
  await db.changeRecord.createMany({
    data: rows.map((change) => {
      ...
      return {
        ...
        changeScope: change?.variantId ? "variant" : "product",
        status: "pending",
        batchId,
      };
    }),
...
async function markWaitingForShopifySlot({
  ...
  await upsertOperationStageProgress({
    ...
    operationType: "BULK_EDIT",
    operationId: historyId,
    workflowStageKey: "WAITING_FOR_SHOPIFY_SLOT",
    stageStatus: "WAITING",
    detail: {
      delayMs,
...
async function processBulkEditExecuteJob(job) {
  ...
  try {
    ...
    try {
      ...
      if (error?.submittedToShopify && error?.shopifyBulkOperationId) {
    ...
    return toWorkerOperationStatusDto({
      ...
      submitted: true,
      historyId,
      shopifyBulkOperationId: submission.shopifyBulkOperationId,
