...
export const unresolvedBulkOperationRecoveryQueue = new Queue(QUEUE_NAME, {
  connection,
  defaultJobOptions: {
    attempts: 2,
...
async function resolveAndDispatch(row) {
  ...
  const edit = await prisma.bulkSubmission.findUnique({
    where: {
  ...
  const undoOwner = await prisma.editHistory.findFirst({
    where: {
      shop,
      undo: {
...
async function registerRepeatableTick() {
  ...
  try {
    await unresolvedBulkOperationRecoveryQueue.add(
      "unresolved-bulk-operation-recovery-tick",
      {},
      {
        jobId: "unresolved-bulk-operation-recovery-tick",
