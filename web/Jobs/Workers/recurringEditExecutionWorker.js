...
import { RECURRING_EDIT_EXECUTION_QUEUE } from "../../queues/adapters/recurringEditQueueAdapter.js";
...
const recurringEditExecutionWorker = new Worker(
  ...
  async (job) => {
    const { recurringEditRunId, shop } = job.data || {};
    if (!recurringEditRunId || !shop) {
      throw new Error("recurring edit execution job requires recurringEditRunId and shop");
    }

    return executeRecurringEditRun(recurringEditRunId, shop);
  },
  {
    connection,
    concurrency: 3,
...
recurringEditExecutionWorker.on("completed", (job, result) => {
  logger.info("Recurring edit execution worker completed job", {
    ...
    jobId: job.id,
    shop: job.data?.shop,
    recurringEditRunId: job.data?.recurringEditRunId,
...
recurringEditExecutionWorker.on("failed", async (job, error) => {
  ...
  if (isRetryExhausted(job)) {
    await recordRetryExhausted({
      ...
      entityId: job?.data?.recurringEditRunId,
      executionId: job?.data?.recurringEditRunId,
