...
export async function runMetafieldBulkWrite({ sessionId, shop }) {
  ...
  for (const batch of chunk(rows, METAFIELDS_SET_BATCH_SIZE)) {
    ...
    try {
      ...
      for (const row of batch) {
        // eslint-disable-next-line no-await-in-loop
        await markRowError(row.id, error?.code || "METAFIELDS_SET_REQUEST_FAILED", true);
        errored += 1;
        processed += 1;
      }
      continue;
    ...
    for (let idx = 0; idx < batch.length; idx += 1) {
      ...
      if (errors.length > 0) {
        ...
        // eslint-disable-next-line no-await-in-loop
        await markRowError(row.id, stale ? "STALE_OBJECT" : firstCode, stale ? false : true);
        errored += 1;
        processed += 1;
        continue;
      }
      ...
      if (!success) {
        ...
        // eslint-disable-next-line no-await-in-loop
        await markRowError(row.id, "METAFIELD_RESULT_MISSING", true);
        errored += 1;
        processed += 1;
        continue;
      }
