...
const subscriptionBillingWorker = new Worker(
  ...
  async (job) => {
    ...
    const commandRow = await prisma.filterTrack.findFirst({
      where: { id: commandId, shop, type: COMMAND_TYPE },
    });
    ...
    if (plan.isFree) {
      const existingSub = await prisma.subscription.findFirst({ where: { shop } });
