...
const automaticProductRuleExecutionWorker = new Worker(
  ...
  async (job) => {
    const { automaticRuleRunId, shop } = job.data || {};
    if (!automaticRuleRunId || !shop) {
      throw new Error("automatic product rule execution job requires automaticRuleRunId and shop");
    }

    return executeAutomaticProductRuleRun(automaticRuleRunId, shop);
  },
  {
    connection,
    concurrency: 3,
...
automaticProductRuleExecutionWorker.on("completed", (job, result) => {
  logger.info("Automatic product rule execution worker completed job", {
    ...
    jobId: job.id,
    shop: job.data?.shop,
    automaticRuleRunId: job.data?.automaticRuleRunId,
...
automaticProductRuleExecutionWorker.on("failed", async (job, error) => {
  ...
  if (isRetryExhausted(job)) {
    await recordRetryExhausted({
      ...
      entityId: job?.data?.automaticRuleRunId,
      executionId: job?.data?.automaticRuleRunId,
