...
const productUpdateWorker = new Worker(QUEUE_NAME, async (job) => {
  ...
  try {
    ...
    if (!mirrorBatchId) {
      ...
      await addShopSyncJob({ shop, syncType: "product", reason: "product_update_missing_active_batch" }).catch(() => {});
    ...
    if (!variants) {
      ...
      await addShopSyncJob({ shop, syncType: "product", reason: "product_update_missing_variants" }).catch(() => {});
    }

    const journal = await applyProductUpsertMutation({ shop, productId: id, mirrorBatchId, mutationType: "PRODUCT_UPDATE", productData, variants, sourceEventAt });
