...
function resolveBulkOperationId(jobData = {}) {
  return jobData.shopifyBulkOperationId || null;
}
...
async function fetchBulkOperationResultUrl({ shop, shopifyBulkOperationId }) {
...
async function findHistoryByBulkOperation({ shop, shopifyBulkOperationId }) {
...
async function processBulkEditResultIngest(job) {
  ...
  const shopifyBulkOperationId = resolveBulkOperationId(job.data || {});
  ...
  const history = await findHistoryByBulkOperation({ shop, shopifyBulkOperationId });
  if (!history) {
  ...
  if (history.cancelRequestedAt) {
    await transitionOperation({
      ...
      metadata: { shopifyBulkOperationId },
      db: db,
    }).catch(() => {});
    return {
      skipped: true,
  ...
  if (
    ...
    const fetched = await fetchBulkOperationResultUrl({ shop, shopifyBulkOperationId });
    ...
    if (
      ...
      const markedShopifyCompleted = await transitionOperation({
        ...
        metadata: { shopifyBulkOperationId, status, webhookStatus },
        dataPatch: {
    ...
    if (shopifyCompleted) {
      const ingesting = await transitionOperation({
        ...
        metadata: { shopifyBulkOperationId, webhookStatus },
        db,
      });
      if (!ingesting?.ok) {
  ...
  if (status && ["FAILED", "CANCELED", "CANCELLED", "EXPIRED"].includes(status)) {
    ...
    const failedUpdate = await transitionOperation({
      ...
      metadata: { shopifyBulkOperationId, shopifyStatus: status, webhookStatus },
      dataPatch: {
        failureStage,
        batch: {
  ...
  if (!resultUrl) {
    ...
    if (exhausted) {
      const failedForMissingUrl = await transitionOperation({
        ...
        metadata: { shopifyBulkOperationId, status, attemptNumber },
        dataPatch: {
  ...
  try {
    ...
    if (deterministicDataError) {
      const failedForDataError = await transitionOperation({
        ...
        metadata: { shopifyBulkOperationId, message },
        dataPatch: {
    ...
    const failedForExpiredUrl = await transitionOperation({
      ...
      metadata: { shopifyBulkOperationId, attemptNumber, message },
      dataPatch: {
        failureStage: "RESULT_INGESTION_URL_EXPIRED",
        batch: mergeBatch(history.batch, {
          resultIngestionFailure: {
...
bulkEditResultIngestWorker.on("completed", (job, result) => {
  logger.info("Bulk edit result ingest worker completed", {
    ...
    jobId: job?.id,
    shop: job?.data?.shop,
    shopifyBulkOperationId: job?.data?.shopifyBulkOperationId,
