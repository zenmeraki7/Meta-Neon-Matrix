...
function assertValidTargetFreezePayload(payload) {
  if (!payload?.shop || !payload?.operationId || !payload?.automaticRuleRunId) {
    throw new Error("target freeze payload requires shop, operationId, and automaticRuleRunId");
  }
}

async function loadRunAndCommand({ shop, automaticRuleRunId, operationId }) {
  const run = await db.automaticProductRuleRun.findFirst({
    where: { id: automaticRuleRunId, shop, operationId },
...
async function processTargetFreezeRequested(payload) {
  ...
  try {
    ...
    return { queued: true, automaticRuleRunId: run.id, commandId: command.id };
  } catch (error) {
