...
async function loadRunAndCommand({ shop, runId, operationId }) {
  const run = await prisma.automaticProductRuleRun.findFirst({
  ...
  const command = await prisma.targetFreezeCommand.findFirst({
    where: {
      shop,
      operationId,
...
async function processTargetFreezeRequested(payload) {
  ...
  const claimed = await prisma.targetFreezeCommand.updateMany({
    where: {
      id: command.id,
      status: TARGET_FREEZE_COMMAND_STATUS.PENDING,
    },
  ...
  try {
    ...
    const movedDispatched = await prisma.targetFreezeCommand.updateMany({
      where: {
        id: command.id,
        status: TARGET_FREEZE_COMMAND_STATUS.DISPATCHING,
      },
    ...
    await prisma.targetFreezeCommand.updateMany({
      where: {
        id: command.id,
        status: TARGET_FREEZE_COMMAND_STATUS.DISPATCHING,
      },
