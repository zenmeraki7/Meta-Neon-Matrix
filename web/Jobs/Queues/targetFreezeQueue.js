...
export async function enqueueTargetFreezeRequestedJob(payload, options = {}) {
  if (!payload?.shop || !payload?.operationId || !payload?.runId) {
    throw new Error("target freeze queue payload requires shop, operationId, and runId");
  }
  ...
  return targetFreezeQueue.add(
    ...
    mergeJobOptions(defaultJobOptions, {
      ...options,
      jobId: options.jobId || joinSafeJobId("target-freeze", payload.operationId, payload.runId),
    }),
  );
}
