...
test("status filter compiles against authoritative normalized status", () => {
  const where = getProductPrismaWhere(
    [{ field: "status", operator: "is", value: "active" }],
    "shop-a",
  );

  const statusClause = where.AND.find((clause) => clause?.statusNormalized);
  assert.ok(statusClause);
  assert.equal(statusClause.statusNormalized.equals, "ACTIVE");
});
