...
function getSampleRate(eventType, overrideRate) {
  if (typeof overrideRate === "number") {
    return Math.min(1, Math.max(0, overrideRate));
  }
  const base = DEFAULT_SAMPLE_RATES[eventType] ?? 0.1;
...
export function emitPerformanceTelemetry(event, options = {}) {
  ...
  const eventType = String(event?.eventType || "web_vital").trim().slice(0, 64);
  const metricName = normalizeMetricName(event?.metricName);
  const sampleRate = getSampleRate(eventType, options.sampleRate);
