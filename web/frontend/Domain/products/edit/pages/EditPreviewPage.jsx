...
export default function EditPreviewPage() {
  ...
  }, [productTargeting.normalizedFilterHash]);

  const matchingProductsQuery = useProducts({
    cursor: matchingCursor,
    rawFilterInput: effectiveFilters,
    cursorFilterHash: productTargeting.normalizedFilterHash,
    limit: 10,
  });
  ...
  const hasFreshPreview = Boolean(
    previewFingerprint?.previewId &&
      previewFingerprint?.normalizedFilterHash &&
    ...
    async (confirmBroadTarget) => {
      const json = await api.post(
        `/api/products/update?lang=${i18n.language}`,
        {
          ...
          previewFilterHash: previewFingerprint?.normalizedFilterHash || null,
