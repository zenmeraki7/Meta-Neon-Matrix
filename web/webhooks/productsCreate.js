...
export default async function handler({ topic, shop, body, deliveryId }) {
  ...
  if (duplicate) {
    console.log("[productsCreate] duplicate delivery skipped", { shop: shopDomain, deliveryId: webhookId });
    return;
  }
  ...
  if (existing.length > 0) {
    console.log("[productsCreate] coalesced away", { shop: shopDomain, productId: productId.toString() });
    return;
  }
