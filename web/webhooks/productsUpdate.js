...
export default async function handler({ topic, shop, body, deliveryId }) {
  ...
  if (duplicate) {
    console.log("[productsUpdate] duplicate delivery skipped", { shop: shopId, deliveryId: webhookId });
    return;
  }
  ...
  if (existing.length > 0) {
    console.log("[productsUpdate] coalesced away", { shop: shopId, productId: productId.toString() });
    return;
  }
