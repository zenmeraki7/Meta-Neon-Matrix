  ...
  async (req, res) => {
    ...
    const result = await prisma.$transaction(async (tx) => {
      const existing = await tx.bulkApplyRequest.findUnique({
        where: {
          shop_bulkJobId_idempotencyKey: { shop, bulkJobId, idempotencyKey },
        },
      });
      ...
      const draft = await tx.bulkEditDraft.findFirst({
        where: { id: bulkJobId, shop, status: "DRAFT" },
      });
    ...
    await applyQueue.add(
      "apply-bulk-edit",
      { shop, bulkJobId, applyRequestId: result.applyRequest.id, idempotencyKey },
      {
