...
function getExecutionTargetsForAutomaticRule({ rule, candidateProducts, candidateTargets }) {
  if (rule.targetResourceType === "VARIANT") {
    ...
    return candidateTargets.filter((target) => target.targetResourceType === "VARIANT");
  }

  return Array.isArray(candidateTargets) && candidateTargets.length
    ? candidateTargets.filter((target) => target.targetResourceType === "PRODUCT")
...
function assertAutomaticRuleExecutionTargets({ rule, targets }) {
  ...
  for (const target of targets) {
    if (!target?.targetResourceType || !target?.targetIdentity || !target?.productId) {
      throw new Error(`Invalid automatic rule target: ${JSON.stringify(target)}`);
    }

    if (rule.targetResourceType === "VARIANT") {
      if (target.targetResourceType !== "VARIANT" || !target.variantId) {
        throw new Error(
    ...
    if (rule.targetResourceType === "PRODUCT") {
      if (target.targetResourceType !== "PRODUCT" || target.variantId) {
        throw new Error(
...
function assertFrozenTargetsMatchRuleScope({ rule, targets }) {
  const expectedTargetType = rule.targetResourceType === "VARIANT" ? "VARIANT" : "PRODUCT";

  for (const target of targets) {
    if (target.targetResourceType !== expectedTargetType) {
      throw new Error(
        `Automatic rule ${rule.id} expected ${expectedTargetType} target but received ${target.targetResourceType}`,
      );
    }
...
function resolveExecutionRuleFromRun({ rule, run }) {
  ...
  return {
    ...
    conditions,
    actions,
    targetResourceType: run?.targetResourceTypeSnapshot || snapshot?.targetResourceType || rule?.targetResourceType,
...
async function enqueueExecutionRun(automaticRuleRunId, shop, opts = {}) {
  return enqueueAutomaticRuleExecution(automaticRuleRunId, shop, opts);
}

async function resolveExistingHistoryForRun(automaticRuleRunId) {
  return findEditHistoryFirst({
    where: {
      automaticProductRuleRunId: automaticRuleRunId,
    },
      select: {
        id: true,
...
async function reserveScheduledRun(ruleId, now) {
  return withAutomaticRuleExecutionTransaction(async (tx) => {
    ...
    // Compatibility backfill for rules created before scheduler state was split
    // from the wide rule definition. New command writes create this eagerly.
    await tx.automaticProductRuleScheduleState.upsert({
      where: {
        shop_automaticProductRuleId: {
          shop: rule.shop,
      ...
      create: {
        ...
        scheduleConfig: rule.scheduleConfig ?? rule.scheduleJson ?? undefined,
        nextRunAt: rule.nextRunAt,
        lastRunAt: rule.lastRunAt,
        disabledAt: rule.schedulerDisabledAt,
      },
      update: {},
    });
    const claimResult = await tx.automaticProductRuleScheduleState.updateMany({
      where: {
        shop: rule.shop,
        automaticProductRuleId: rule.id,
        ruleRevision: rule.revision,
    ...
    const executionDedupeKey = buildScheduledExecutionKey(rule.id, scheduledFor);
    const existingRun = await automaticProductRuleRunRepository.findByExecutionKeyForShop(executionDedupeKey, rule.shop, tx);

    if (existingRun) {
      await tx.automaticProductRuleScheduleState.updateMany({
        where: {
          shop: rule.shop,
          automaticProductRuleId: rule.id,
          claimOwner: schedulerClaimId,
      ...
      return {
        automaticRuleRunId: existingRun.id,
        executionDedupeKey,
        shop: rule.shop,
        reused: true,
      };
...
async function markRunFailed({ run, rule, errorMessage, processingToken = null, data = {} }) {
  if (!processingToken) {
    logger.warn("Automatic rule run failure transition skipped due to missing processing token", {
      automaticRuleRunId: run?.id || null,
...
export async function enqueueAutomaticProductRuleExecutionJob({ automaticRuleRunId, shop }) {
  return enqueueExecutionRun(automaticRuleRunId, shop);
}
...
export async function reserveAutomaticProductRuleRunFromSignal({
  ...
  for (const rule of rules) {
    const executionDedupeKey = buildSignalExecutionKey(rule.id, triggerSource, triggerReference, productIds);

    try {
      const existingRun = await automaticProductRuleRunRepository.findByExecutionKeyForShop(executionDedupeKey, shop);
      if (existingRun) {
  ...
  try {
    ...
    for (const { id } of dueIds) {
      try {
        ...
        if (!reservation?.automaticRuleRunId) {
          skipped += 1;
          continue;
        }

        await enqueueExecutionRun(reservation.automaticRuleRunId, reservation.shop);
        scheduled += 1;
      } catch (error) {
        if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") {
          skipped += 1;
...
export async function executeAutomaticProductRuleRun(automaticRuleRunId, shopFromJob = null) {
  let run = await automaticProductRuleRunRepository.findByIdWithRuleUnsafeInternal(automaticRuleRunId);
  ...
  try {
    run = await automaticProductRuleRunRepository.findByIdWithRuleForShop(automaticRuleRunId, initialRule.shop);
    ...
    const automaticRuleTargetType = executionRule.targetResourceType === "VARIANT" ? "VARIANT" : "PRODUCT";
    ...
    try {
      const baseHistory = await bulkService._bulkOperationEdit(
        {
          rawFilterInput: executionRule.conditions,
          productIds,
          variantIds,
          targetResourceType: automaticRuleTargetType,
      ...
      const prepared = await withAutomaticRuleExecutionTransaction(async (tx) => {
        ...
        const frozenTransition = await tx.editHistory.updateMany({
          ...
          data: {
            ...
            targetProductMirrorBatchId: run.mirrorBatchId,
      ...
      await automaticProductRuleRunRepository.updateProcessingCountsForShop(run.id, rule.shop, {
        ...
        targetingSnapshotMeta: {
          ...
          normalizedFilterHash: crypto.createHash("sha256")
            ...
            .digest("hex"),
          targetSetHash: prepared.targetSetHash || null,
          targetGranularity: automaticRuleTargetType,
          resolvedAt: new Date(),
        },
      });
...
export async function finalizeAutomaticProductRuleRunFromHistory({
  ...
  if (!tokenForTransition) {
    logger.warn("Automatic rule run finalization skipped due to missing processing token", {
      automaticRuleRunId: history.automaticProductRuleRunId,
      historyId,
      shop: run.shop,
