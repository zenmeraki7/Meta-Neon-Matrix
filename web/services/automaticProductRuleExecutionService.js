...
import { buildAutomaticRuleRunJobId } from "../utils/jobQueueUtils.js";
import {
  buildActorContext,
...
export async function reserveAutomaticRuleApplications({
  ...
  return reserveAutomaticRuleApplicationsViaService({
    createApplication: (data) => prisma.automaticRuleApplication.create({ data }),
    rule,
    run,
    candidateTargets,
    candidateProducts,
...
async function enqueueExecutionRun(runId, shop, opts = {}) {
  ...
  let executionKey = opts.executionKey || null;

  if (!ruleId || !executionKey) {
    const run = await automaticProductRuleRunRepository.findByIdForShop(runId, shop);
    ruleId = ruleId || run?.automaticProductRuleId || runId;
    executionKey = executionKey || run?.executionKey || runId;
  }
...
export async function executeAutomaticProductRuleRun(runId, shopFromJob = null) {
  ...
  try {
    ...
    try {
      ...
      const queued = await prisma.editHistory.updateMany({
        where: {
          id: editHistoryId,
          shop: rule.shop,
          executionState: OPERATION_LIFECYCLE_STATES.TARGET_FROZEN,
