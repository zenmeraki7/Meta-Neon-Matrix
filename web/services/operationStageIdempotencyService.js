...
export async function beginEditHistoryStage({
  ...
  if (
    ...
    return {
      state: "stale",
      workflowStageKey: buildStageKey(shop, history.executionIdentity, stage),
  ...
  const workflowStageKey = buildStageKey(shop, operationId, stage);
  ...
  if (existing.status === "completed") {
    return { state: "completed", workflowStageKey, operationId, stageState: existing };
  }
  ...
  return { state: "started", workflowStageKey, operationId, stageState: nextStageState };
}
