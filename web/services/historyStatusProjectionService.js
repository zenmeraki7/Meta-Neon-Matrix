...
function mapBulkUndoSummary(undoValue) {
  ...
  const state = undo.executionState || undo.state || BULK_UNDO_STATES.PLANNED;
  ...
  switch (state) {
    ...
    default: {
      ...
      const legacyStatus = String(undo.outcomeStatus || undo.status || "").toLowerCase();
...
export function projectEditHistoryStatus(record) {
  ...
  return {
    ...
    supportStatus: {
      ...
      targetProductMirrorBatchId: record.targetProductMirrorBatchId || null,
      errors,
      lastError: errors[errors.length - 1] || null,
      undoState: undo.executionState || undo.state || null,
