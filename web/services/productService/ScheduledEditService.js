...
export class ScheduledEditService {
  ...
  async createScheduledEdit(input = {}) {
    ...
    requireMatchingPreviewValue({
      ...
      expected: fingerprint.normalizedFilterHash,
    ...
    const target = await TargetingEngineService.resolvePreviewTargets({
      shop: this.session.shop,
      source: "SCHEDULED",
      targetResourceType: isVariantLevelField(editedField) ? "VARIANT" : "PRODUCT",
      targetGranularity: isVariantLevelField(editedField) ? "VARIANT" : "PRODUCT",
      filterAst: filterAst ?? null,
      legacyFilterParams: Array.isArray(rawFilterInput) ? rawFilterInput : [],
    ...
    const history = await db.editHistory.create({
      data: {
        ...
        targetProductMirrorBatchId: target.mirrorBatchId || fingerprint.mirrorBatchId || null,
        normalizedFilterHash: target.normalizedFilterHash || fingerprint.normalizedFilterHash || null,
        scheduledAt,
        scheduledUndoAt,
        editType: "Scheduled edit",
        legacyQueryFilter: JSON.stringify(target.where),
        rules: [
          {
            field: editedField,
            value,
        ...
        batch: {
          ...
          previewFingerprint: {
            previewId: resolvedPreviewContractId,
            normalizedFilterHash: String(fingerprint.normalizedFilterHash || ""),
    ...
    await createEnqueueIntent({
      ...
      dispatchDedupeKey: `scheduled-edit:${this.session.shop}:${history.id}:${scheduledAt.toISOString()}`,
    });
    ...
    if (scheduledUndoAt && scheduledUndoAt.getTime() > Date.now() && undoAllowed) {
      ...
      if (undoDelay > 0) {
        await createEnqueueIntent({
          ...
          dispatchDedupeKey: `scheduled-undo:${this.session.shop}:${history.id}:${scheduledUndoAt.toISOString()}`,
        });
      }
    }
