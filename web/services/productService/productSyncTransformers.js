...
function parseFixedSix(value) {
  ...
  let units = BigInt(match[2]) * MONEY_SCALE + BigInt(fraction.slice(0, 6));
  if (fraction[6] >= "5") units += 1n;
  return sign * units;
}

function formatFixedSix(units) {
  const sign = units < 0n ? "-" : "";
  const absolute = units < 0n ? -units : units;
  return `${sign}${absolute / MONEY_SCALE}.${String(absolute % MONEY_SCALE).padStart(6, "0")}`;
}

function normalizeNullableDecimal(value) {
  const units = parseFixedSix(value);
...
export function flattenVariant(productId, variant, shop) {
  ...
  return {
    ...
    weight: normalizeNullableDecimal(
