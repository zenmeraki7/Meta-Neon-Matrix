...
function buildPrismaStatusFilter(operator, value) {
  ...
  switch (operator) {
    ...
    case "is not":
      return { NOT: normalizedFieldMatch };
    case "is empty":
    case "is empty/blank":
      return { statusNormalized: { equals: "UNKNOWN" } };
    case "is not empty":
      return { statusNormalized: { not: "UNKNOWN" } };
    default:
      return normalizedFieldMatch;
  }
}
...
function enforceOptionValueNamePairing(rawFilterInput = []) {
  const hasOptionName1 = rawFilterInput.some(
    (f) => f?.field === "option_name_1" && hasMeaningfulFilterValue(f),
  );
  const hasOptionName2 = rawFilterInput.some(
    (f) => f?.field === "option_name_2" && hasMeaningfulFilterValue(f),
  );
  const hasOptionName3 = rawFilterInput.some(
...
export function getProductPrismaWhere(rawFilterInput = [], shop) {
  ...
  enforceOptionValueNamePairing(rawFilterInput);

  const where = { shop };
  const AND = [];

  for (const rawFilter of rawFilterInput) {
