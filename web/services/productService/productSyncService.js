...
export async function startBulkOperationToFetchProducts({
  ...
  const { shopifyBulkOperationId, responseBody } =
...
export async function formatAndSyncProductsToDB({
  ...
  try {
    ...
    await stageProductMirrorBatch({ shop, mirrorBatchId, syncHistoryId });
    ...
    const hasExistingMirror = Boolean(previousState?.currentProductMirrorBatchId);
