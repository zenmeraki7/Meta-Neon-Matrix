...
function normalizeFilters(body = {}) {
  if (body.rawFilterInput != null && !Array.isArray(body.rawFilterInput)) {
    throw buildValidationError("rawFilterInput must be an array", "INVALID_FILTER_PARAMS");
  }
  ...
  const filters = Array.isArray(body.rawFilterInput)
    ? body.rawFilterInput
...
function normalizeProductQueryRequest(query = {}, body = {}) {
  ...
  const rawFilterInput = normalizeFilters(safeBody);
...
async function trackFilterQueryIfPossible({ shop, rawFilterInput, count }) {
...
export async function executeProductQuery({ shop, query = {}, body = {} }) {
  ...
  try {
    result = await productService.getProductsWithFilters({
      ...
      rawFilterInput: normalized.rawFilterInput,
      shop,
    });
  } catch (error) {
    ...
    result = {
      ...
      mirrorBatchId: mirrorHealth?.currentProductMirrorBatchId || null,
  ...
  return {
    ...result,
    shopDomain: String(shop || "").trim().toLowerCase(),
    mirrorHealth,
  };
}
