...
function buildVariantKeysetWhere(sortKey, sortOrder, cursor) {
  ...
  if (sortKey === "PRICE" && cursor.price !== null && cursor.price !== undefined) {
    return {
      OR: [
        { price: { [cmp]: new Prisma.Decimal(cursor.price) } },
        {
          AND: [
            { price: new Prisma.Decimal(cursor.price) },
            { id: { [cmp]: cursor.id } },
          ],
        },
      ],
...
async function resolveCollectionMatchedIds({
  ...
  const containsExpr = (() => {
    if (collectionId) {
      return Prisma.sql`c."shopifyId" = ${collectionId}`;
    }
    if (handle) {
      return Prisma.sql`LOWER(COALESCE(c."handle", '')) LIKE LOWER(${`%${handle}%`})`;
    }
    return Prisma.sql`LOWER(COALESCE(c."title", '')) LIKE LOWER(${likeValue})`;
  })();
    ...
    if (handle) {
      if (normalizedOperator === "contains" || normalizedOperator === "does not contain") {
        if (normalizedOperator === "does not contain" && negativeOperatorMode === NEGATIVE_OPERATOR_MODE.NOT_LIKE) {
          return Prisma.sql`LOWER(COALESCE(c."handle", '')) NOT LIKE LOWER(${`%${handle}%`})`;
        }
        return Prisma.sql`LOWER(COALESCE(c."handle", '')) LIKE LOWER(${`%${handle}%`})`;
      }
      return Prisma.sql`LOWER(COALESCE(c."handle", '')) = LOWER(${handle})`;
    }
    if (normalizedOperator === "contains" || normalizedOperator === "does not contain") {
      if (normalizedOperator === "does not contain" && negativeOperatorMode === NEGATIVE_OPERATOR_MODE.NOT_LIKE) {
        return Prisma.sql`LOWER(COALESCE(c."title", '')) NOT LIKE LOWER(${likeValue})`;
      }
      return Prisma.sql`LOWER(COALESCE(c."title", '')) LIKE LOWER(${likeValue})`;
    }
    return Prisma.sql`LOWER(COALESCE(c."title", '')) = LOWER(${title})`;
  })();
...
async function resolveMetafieldMatchedIds({
  ...
  const valueClause = (() => {
    if (normalizedOperator === "is empty" || normalizedOperator === "is empty/blank") {
      return Prisma.sql`(m."valueTextNormalized" IS NULL OR m."valueTextNormalized" = '')`;
    }
    if (normalizedOperator === "is not empty") {
      return Prisma.sql`(m."valueTextNormalized" IS NOT NULL AND m."valueTextNormalized" <> '')`;
    }
    if ((normalizedOperator === ">" || normalizedOperator === ">=" || normalizedOperator === "<" || normalizedOperator === "<=") && !Number.isNaN(parsedNumber)) {
      ...
      return Prisma.sql`m."valueNumber" ${Prisma.raw(op)} ${parsedNumber}`;
    }
    if ((normalizedOperator === "is before" || normalizedOperator === "is after") && parsedDate) {
      const op = normalizedOperator === "is before" ? "<" : ">";
      return Prisma.sql`m."valueDate" ${Prisma.raw(op)} ${parsedDate}`;
    }
    if ((normalizedOperator === "is" || normalizedOperator === "equals") && parsedBoolean !== null) {
      return Prisma.sql`m."valueBoolean" = ${parsedBoolean}`;
    }
    if (normalizedOperator === "contains" || normalizedOperator === "does not contain") {
      if (normalizedOperator === "does not contain" && negativeOperatorMode === NEGATIVE_OPERATOR_MODE.NOT_LIKE) {
        return Prisma.sql`COALESCE(m."valueTextNormalized", '') NOT LIKE ${likeValue}`;
      }
      return Prisma.sql`COALESCE(m."valueTextNormalized", '') LIKE ${likeValue}`;
    }
    return Prisma.sql`COALESCE(m."valueTextNormalized", '') = ${normalizedValue}`;
  })();
...
export async function resolveCanonicalVariantTarget({
  ...
  const fetchLimit = clampLimit(limit, sampleLimit, sampleLimit);
  const [count, rows] = await prisma.$transaction([
    prisma.variant.count({ where: variantWhere }),
