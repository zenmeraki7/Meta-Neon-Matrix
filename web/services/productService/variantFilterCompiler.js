...
export function compileVariantPrismaWhere(filterParams = [], shop, mirrorBatchId) {
...
export function splitProductAndVariantFilters(filterParams = []) {
...
export async function resolveVariantMetafieldMatchedVariantIds({
  ...
  const valueClause = (() => {
    ...
    if ([">", ">=", "<", "<="].includes(normalizedOperator) && !Number.isNaN(parsedNumber)) {
      return Prisma.sql`m."valueNumber" ${Prisma.raw(normalizedOperator)} ${parsedNumber}`;
    }
