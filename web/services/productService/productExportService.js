...
export class ProductExportService {
  ...
  async getAllExportHistories({
    ...
    if (normalizedType.includes("scheduled")) {
      where.exportType = { in: ["Scheduled export", "scheduled export", "Scheduled Export"] };
    } else if (normalizedType.includes("manual")) {
      where.exportType = { in: ["Manual export", "manual export", "Manual Export"] };
    }
    ...
    const items = histories.map((history) =>
      projectExportHistoryStatus({
        ...
        rawType: history.exportType || "",
        type: EXPORT_TYPES[history.exportType]?.[lang] || history.exportType || "",
      }),
    );
  ...
  async createExportJob({
    ...
    const begin = await this.idempotencyStore.begin({
      ...
      requestHash: buildIdempotencyRequestHash({
        ...
        selectedFieldKeys: normalizedFields,
        fileName,
        rawFilterInput,
        filterAst,
        targetGranularity,
