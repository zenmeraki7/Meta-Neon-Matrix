...
function buildProductListCacheKey({
  ...
  const normalizedFilterHash = crypto
    .createHash("sha256")
    .update(stableJson(rawFilterInput || []))
    .digest("hex");
...
export async function getDistinctProductFilterValues({
  ...
  if (fieldConfig.source === "product") {
    ...
    const currentCollectionMirrorBatchId = await findStoreActiveCollectionBatchId(shop);
    rows = await findDistinctCollectionTitles({
      shop,
      mirrorBatchId: currentCollectionMirrorBatchId,
      search,
      take,
    });
