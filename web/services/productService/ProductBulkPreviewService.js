...
async function resolveBestAvailableMirrorBatchId(shop) {
  ...
  if (state?.currentProductMirrorBatchId) {
    return {
      mirrorBatchId: state.currentProductMirrorBatchId,
      mirrorState: state,
    };
  }
...
export class ProductBulkPreviewService {
  ...
  async resolvePreviewTargetsWithSnapshotFallback({
    ...
    try {
      return await TargetingEngineService.resolvePreviewTargets({
        ...
        targetResourceType: isVariant ? "VARIANT" : "PRODUCT",
  ...
  async resolvePreviewTargetsFromAvailableSnapshot({
    ...
    if (!previewFilterAst && (!Array.isArray(rawFilterInput) || rawFilterInput.length === 0)) {
      throw buildPreviewTargetingError();
    }
    ...
    if (isVariant) {
      ...
      return {
        ...
        normalizedFilterHash: prepared.normalizedFilterHash,
  ...
  async trackEditProducts({
    ...
    const previewSignatureHash = crypto
      ...
      .update(JSON.stringify({
        ...
        normalizedFilterHash: target.normalizedFilterHash,
    ...
    return {
      executionPlan: {
        ...
        targetResourceType: executionPlan.targetResourceType,
