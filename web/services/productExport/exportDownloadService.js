...
export async function findDownloadableExport({ exportJobId, shop, db = defaultDb }) {
  ...
  return {
    ...exportJob,
    fileUrl: validateStoredCloudinaryUrl(exportJob.fileUrl),
    filename: sanitizeAttachmentFilename(exportJob.filename),
  };
}
...
export async function streamExportCsvDownload({
  ...
  try {
    const upstream = await fetchImpl(exportJob.fileUrl, {
      method: "GET",
      redirect: "follow",
