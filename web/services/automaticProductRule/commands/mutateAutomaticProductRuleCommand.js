...
function buildRevisionSnapshot(rule) {
  return {
    ...
    filterAst: rule.normalizedFilterAst || rule.filterAst,
    editOperation: rule.editOperationJson,
    schedule: rule.scheduleConfig || rule.scheduleJson,
  };
}

async function persistRuleRevisionAndSchedule(tx, rule) {
  await tx.automaticProductRuleRevision.upsert({
    where: {
      shop_automaticProductRuleId_revision: {
        shop: rule.shop,
    ...
    create: {
      ...
      commandSnapshot: buildRevisionSnapshot(rule),
      createdByActorId: rule.updatedByActorId || rule.createdByActorId,
    },
    update: {},
  });
  ...
  await tx.automaticProductRuleScheduleState.upsert({
    ...
    create: {
      ...
      scheduleConfig: rule.scheduleConfig || rule.scheduleJson,
...
async function cancelQueuedRunsForRule({ tx, shop, ruleId, actor, reason }) {
  ...
  await tx.automaticProductRuleRun.updateMany({
    ...
    data: {
      ...
      cancelledAt: now,
      cancelledByActorId: actorRef(actor),
      cancelReason: reason,
      updatedAt: now,
    },
  });
...
export async function createAutomaticProductRule({ shop, actor, command, entitlement }) {
  ...
  return db.$transaction(async (tx) => {
    ...
    await persistRuleRevisionAndSchedule(tx, rule);
...
export async function updateAutomaticProductRule({
  ...
  return db.$transaction(async (tx) => {
    ...
    await persistRuleRevisionAndSchedule(tx, updatedRule);
    ...
    await createAuditEvent({
      ...
      metadata: {
        ...
        ruleConfigHash: updatedRule.ruleConfigHash,
      },
    });
  ...
  return db.$transaction(async (tx) => {
    ...
    const updateResult = await tx.automaticProductRule.updateMany({
      ...
      data: {
        ...
        pausedAt: now,
        pausedByActorId: actorRef(safeActor),
        schedulerDisabledAt: now,
        updatedAt: now,
        updatedByActorId: actorRef(safeActor),
        revision: {
          increment: 1,
        },
      },
  ...
  return db.$transaction(async (tx) => {
    ...
    const updateResult = await tx.automaticProductRule.updateMany({
      ...
      data: {
        ...
        resumedAt: now,
        resumedByActorId: actorRef(safeActor),
...
export async function softDeleteAutomaticProductRule({
  ...
  const safeActor = assertActor(actor, safeShop);
  const safeRuleId = assertRuleId(automaticProductRuleId);
  const deletePolicy = deleteCommand?.deletePolicy || DELETE_POLICY.BLOCK_FUTURE_RUNS;
