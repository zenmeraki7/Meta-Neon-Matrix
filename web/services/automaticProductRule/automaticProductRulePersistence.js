...
export function computeConfigFingerprintFromCommand(command) {
  return sha256Json({
...
export function computeConfigFingerprintFromRuleLike(ruleLike) {
  return sha256Json({
    commandVersion: ruleLike.commandVersion ?? 1,
    targetMode: ruleLike.targetMode,
    filterAst: ruleLike.filterAstJson ?? ruleLike.filterAst ?? null,
...
export function normalizeRuleCreateData({ shop, actor, command }) {
  ...
  const configFingerprint = computeConfigFingerprintFromCommand(command);
  const now = new Date();
  ...
  return {
    ...
    commandVersion: command.commandVersion ?? 1,
    targetMode: command.targetMode,
    filterAstJson: command.filterAst ?? null,
    savedTargetSetId: command.savedTargetSetId ?? null,
...
export function buildUpdateDataFromPatch({ existingRule, patchCommand, actor }) {
  ...
  const nextRuleLike = {
    ...
    targetMode: patchCommand.targetMode ?? existingRule.targetMode,
    filterAstJson: patchCommand.filterAst !== undefined ? patchCommand.filterAst : existingRule.filterAstJson,
  ...
  const data = {
    ...
    configFingerprint: computeConfigFingerprintFromRuleLike(nextRuleLike),
  };
  ...
  if (patchCommand.filterAst !== undefined) data.filterAstJson = patchCommand.filterAst;
  if (patchCommand.savedTargetSetId !== undefined) data.savedTargetSetId = patchCommand.savedTargetSetId;
  if (patchCommand.targetMode !== undefined) data.targetMode = patchCommand.targetMode;
