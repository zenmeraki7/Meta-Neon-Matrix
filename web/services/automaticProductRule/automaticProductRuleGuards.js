...
export function assertValidStateTransition({ from, to }) {
  const allowed = {
    [RULE_STATUS.DRAFT]: [RULE_STATUS.ACTIVE, RULE_STATUS.PAUSED],
    [RULE_STATUS.ACTIVE]: [RULE_STATUS.PAUSED],
    [RULE_STATUS.PAUSED]: [RULE_STATUS.ACTIVE],
  };
...
export async function assertMirrorSafeForTargeting({ tx, shop }) {
  ...
  if (store.mirrorHealthState === "UNSAFE" || store.isProductInitiallySyncing === true) {
