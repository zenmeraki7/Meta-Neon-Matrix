...
export async function startProductSync(command = Object.freeze({})) {
  ...
  const productCount = await getActiveProductCountByShop(shop, store?.currentProductMirrorBatchId);

  const alreadySynced =
    !!store
    && Boolean(store.currentProductMirrorBatchId)
    && store.isProductSyncing === false
    && store.isProductInitiallySyncing === false
  ...
  if (alreadySynced && !force) {
    return {
      ...
      data: {
        ...
        lastCompletedMirrorBatchId: latestCompletedSync?.mirrorBatchId || null,
      },
    };
  }
