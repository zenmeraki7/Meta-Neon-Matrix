...
class AdminService {
  ...
  async getCompletedEditHistorySummary() {
    ...
    const storeMap = new Map(stores.map((s) => [
      s.shopUrl,
      s.installationStatus === "UNINSTALLED",
    ]));
  ...
  async getStoreStats() {
    const [totalStores, installedStores, uninstalledStores] =
      await Promise.all([
        ...
        db.store.count({
          where: { installationStatus: "UNINSTALLED" },
        }),
      ]);
  ...
  async getStoreDetails(shopUrl) {
    ...
    return {
      ...
      stats: {
        ...
        lastEditDate: lastEdit?.requestedAt || null,
  ...
  async getEditHistoryList({
    ...
    if (type !== "all") where.editType = type;

    // Whitelist sortable fields
    const sortFieldMap = {
      editTime: "requestedAt",
      createdAt: "createdAt",
      updatedAt: "updatedAt",
    };
    ...
    return {
      editHistories: editHistories.map((history) => ({
        ...history,
        editTime: history.requestedAt || history.createdAt || null,
      })),
      pagination: {
        hasNextPage,
        nextCursor,
  ...
  async getFailedEdits({ cursor = null, limit = 20, shopUrl = null }) {
    ...
    const cursorFilter = cursorPayload?.editTime && cursorPayload?.id
      ? {
          OR: [
            { requestedAt: { lt: new Date(cursorPayload.editTime) } },
            { AND: [{ requestedAt: new Date(cursorPayload.editTime) }, { id: { lt: cursorPayload.id } }] },
          ],
        }
      : null;
    ...
    const nextCursor = last
      ? this.encodeCursor({ id: last.id, editTime: last.requestedAt ? new Date(last.requestedAt).toISOString() : null })
      : null;
    ...
    return {
      failedEdits: failedEdits.map((history) => ({
        ...history,
        editTime: history.requestedAt || history.createdAt || null,
      })),
      errorGroups,
  ...
  async getRecentActivity(limit = 10) {
    ...
    return {
      recentEdits: recentEdits.map(({ editType, requestedAt, ...edit }) => ({
        ...edit,
        type: editType,
        editTime: requestedAt,
      })),
