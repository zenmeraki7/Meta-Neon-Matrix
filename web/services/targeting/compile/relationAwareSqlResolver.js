...
function buildCollectionsExpr({ targetResourceType, operator, value, params }) {
  ...
  const targetProductExpr = targetResourceType === "VARIANT" ? `v."productId"` : `p."id"`;
  const existsSql = `
    EXISTS (
      SELECT 1
...
function buildMetafieldExpr({ targetResourceType, variantScoped, operator, value, meta, params }) {
  ...
  const ownerIdExpr = variantScoped
    ? (targetResourceType === "VARIANT" ? `v."id"` : `vx."id"`)
    : (targetResourceType === "VARIANT" ? `v."productId"` : `p."id"`);
  const ownerJoin = variantScoped && targetResourceType === "PRODUCT"
...
function compilePredicate(node, context, params) {
  ...
  if (context.targetResourceType === "PRODUCT" && isVariantField) {
  ...
  const alias = context.targetResourceType === "VARIANT"
    ? (isProductField ? "p" : "v")
    : "p";
