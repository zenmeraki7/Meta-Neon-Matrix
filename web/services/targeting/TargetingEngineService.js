...
function resolveInputAst({
  ...
  return adaptLegacyFilterParamsToAst({
    rawFilterInput: legacyFilterParams || [],
    targetGranularity,
    source,
  });
}
...
function buildTargetingParityError({
  ...
  error.meta = {
    ...
    previewTargetSetHash: String(canonicalResult?.targetSetHash || ""),
    alternateTargetSetHash: String(alternateResult?.targetSetHash || ""),
    ...
    filterFingerprint: normalizedFilterHash || null,
  };
  return error;
}
...
async function computeOrderedTargetIdentityDigestFromSql({
  ...
  const productExpr = targetResourceType === "VARIANT" ? `v."productId"` : `p."id"`;
  const variantExpr = targetResourceType === "VARIANT" ? `v."id"` : `NULL`;
  const baseFrom =
    targetResourceType === "VARIANT"
      ? `FROM "Variant" v
  ...
  while (true) {
    ...
    cursorVariantId =
      targetResourceType === "VARIANT" ? String(last?.variantId || "") : null;
    if (rows.length < PAGE_SIZE) break;
  }

  return {
    count,
    targetSetHash: hash.digest("hex"),
    firstIds,
    lastIds: tailIds,
  };
}
...
function assertLegacyAdapterAllowed({
  ...
  if (freeze && allowLegacyFilterParams !== true) {
    throw new TargetingValidationError(
      "Legacy rawFilterInput adapter is read-only; filterAst is required for writes",
...
async function persistTargetingMetadata({
  ...
  const baseSnapshotMeta = {
    ...
    mirrorBatchId,
    targetCount,
    normalizedFilterHash: payload.normalizedFilterHash,
    normalizedAst: payload.normalizedFilterAst,
    source: payload.source,
    resolvedAt,
    freezeKey,
    freezeStats: freezeStats || null,
    targetSetHash: freezeStats?.targetSetHash || null,
  ...
  const data = {
    ...
    targetFreezeMode: targetFreezeModeOverride || "STATIC",
    targetGranularity: payload.targetGranularity,
  ...
  if (ownerType === "EXPORT_JOB") {
    data.targetProductMirrorBatchId = mirrorBatchId;
    data.legacyFilterQuery = JSON.stringify(resolvedWhere || {});
  }
  if (ownerType === "EDIT_HISTORY") {
    data.targetProductMirrorBatchId = mirrorBatchId;
    data.legacyQueryFilter = JSON.stringify(resolvedWhere || {});
  }
...
async function resolveAndMaybeFreeze({
  ...
  if (!isEngineV2EnabledForFlow(flags, flow) && !filterAst) {
    ...
    const resolved = await resolveCanonicalTarget({
      shop,
      targetResourceType,
      rawFilterInput: Array.isArray(legacyFilterParams) ? legacyFilterParams : [],
      queryParams,
      sampleLimit,
      freeze: false,
    });
      ...
      if (modelName && db[modelName]) {
        ...
        await db[modelName]
          .updateMany({
            ...
            data: {
              normalizedFilterHash: legacyFilterHash,
              ...
              ...(ownerType === "EXPORT_JOB"
                ? {
                    targetProductMirrorBatchId: resolved.mirrorBatchId,
                    legacyFilterQuery: JSON.stringify(resolved.where || {}),
                  }
                : {}),
              ...(ownerType === "EDIT_HISTORY"
                ? {
                    targetProductMirrorBatchId: resolved.mirrorBatchId,
                    legacyQueryFilter: JSON.stringify(resolved.where || {}),
                  }
                : {}),
            },
          })
  ...
  const normalizedFilterHash = hashFilterAst(normalized.normalizedFilterAst);
  ...
  const compiled = hasRelationPredicates
    ? {
        where: null,
        targetModel: targetResourceType === "VARIANT" ? "Variant" : "Product",
        targetGranularity: normalized.targetGranularity,
        orderBy: [{ id: "asc" }],
      }
    : compileWithScope({
  ...
  try {
    if (hasRelationPredicates) {
      const targetModel = targetResourceType === "VARIANT" ? "Variant" : "Product";
  ...
  if (shouldRunParity && paritySqlText && Array.isArray(paritySqlParams)) {
    ...
    const canonicalResult = {
      ...
      targetSetHash: hashValues(canonicalSampleIds),
  ...
  let targetResourceTypeDistribution = null;
  ...
  if (freeze) {
    ...
    if (modelName && db[modelName]) {
      ...
      if (
        ...
        String(existingSnapshotMeta?.normalizedFilterHash || "") ===
          String(normalizedFilterHash || "")
      ) {
        reusedSnapshot = {
    ...
    freezeStats.targetSetHash = await computeTargetSetHash({
      ownerType,
      ownerId,
      shop,
      mirrorBatchId: resolved.mirrorBatchId,
    ...
    targetResourceTypeDistribution = await computeTargetTypeDistribution({
      db,
      shop,
      ownerType,
      ownerId,
    ...
    const distributionTotal =
      Number(targetResourceTypeDistribution.PRODUCT || 0) +
      Number(targetResourceTypeDistribution.VARIANT || 0);
    if (distributionTotal !== Number(freezeStats.finalSnapshotCount || 0)) {
      throw new TargetingValidationError("TARGET_TYPE_DISTRIBUTION_MISMATCH", {
        ...
        meta: {
          distributionTotal,
          finalSnapshotCount: Number(freezeStats.finalSnapshotCount || 0),
          targetResourceTypeDistribution,
          shop,
          ownerType,
          ownerId,
          mirrorBatchId: resolved.mirrorBatchId,
  ...
  return {
    ...
    parity: {
      count: Number(resolved.count || 0),
      sampleChecksum,
      freezeChecksum: freezeStats?.targetSetHash || null,
      productTargetChecksum: productTargetChecksum || null,
      variantTargetChecksum: variantTargetChecksum || null,
      targetResourceTypeDistribution: targetResourceTypeDistribution || null,
      mirrorBatchId: resolved.mirrorBatchId,
      normalizedFilterHash,
    },
...
export const TargetingEngineService = {
  ...
  async resolveAndFreezeExportTargets(input) {
    return resolveAndMaybeFreeze({
      ...
      allowLegacyFilterParams: false,
      targetFreezeModeOverride:
        input?.targetFreezeModeOverride || TARGETING_MODES.DYNAMIC_AT_RUN,
  ...
  async resolveAndFreezeScheduledTargets(input) {
    return resolveAndMaybeFreeze({
      ...
      freeze: true,
      allowLegacyFilterParams: false,
      targetFreezeModeOverride: TARGETING_MODES.STATIC_AT_SCHEDULE_CREATE,
      targetingSnapshotMetaOverride: {
  ...
  async resolveAndFreezeRecurringRunTargets(input) {
    ...
    return resolveAndMaybeFreeze({
      ...
      targetFreezeModeOverride: TARGETING_MODES.DYNAMIC_AT_RUN,
      targetingSnapshotMetaOverride: {
