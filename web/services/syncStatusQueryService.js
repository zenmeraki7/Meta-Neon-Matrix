...
function normalizeStoreSyncState(store) {
  const source = store || DEFAULT_STORE_SYNC_STATE;
  const stage = source.syncProgressStage || "IDLE";
  const active = stage !== "IDLE";
  return {
    ...source,
    syncProgressStage: stage,
    // Compatibility booleans are derived at the API boundary from the
    // authoritative persisted stage, so contradictory stored projections do
    // not leak into the UI.
    isProductSyncing: active,
    isProductInitiallySyncing:
      active && source.isProductInitiallySyncing === true,
    hasCompletedShopifyBulkJob:
      stage === "SHOPIFY_BULK_RUNNING"
        ? false
        : active
          ? true
          : source.hasCompletedShopifyBulkJob === true,
  };
}
...
function toSyncStatusDetailDto(store, latestSync, latestCompletedSync, productCount) {
  ...
  return {
    ...
    isProductInitiallySyncing: storeState.isProductInitiallySyncing,
    productInitialSyncProgress: storeState.productInitialSyncProgress,
    hasCompletedShopifyBulkJob: storeState.hasCompletedShopifyBulkJob,
...
export async function getTrackedProductSyncStatus({ session, shop }) {
  ...
  if (
    ...
    storeDetails.isProductInitiallySyncing === false
  ) {
    if (!storeDetails.currentProductMirrorBatchId) {
      return {
        success: false,
        message:
  ...
  if (!storeDetails.hasCompletedShopifyBulkJob) {
    if (!latestSync?.shopifyBulkOperationId) {
      return {
        success: true,
    ...
    const result = await getBulkEditStatus(latestSync.shopifyBulkOperationId, session);
