...
import { assertSupportedExportFields } from "./productService/productExportFieldRegistry.js";
...
function validateFields(fields = [], targetGranularity = "PRODUCT") {
  ...
  if (canonicalFields.length > 128) {
    throw new Error("A scheduled export cannot contain more than 128 fields");
  }
  assertSupportedExportFields(canonicalFields, { targetGranularity });
...
function serializeScheduledExport(item, countsById = {}, latestRunsById = {}) {
  ...
  return {
    ...
    fields: item.selectedFieldKeys,
    filename: item.generatedFilename,
    rawFilterInput: item.rawFilterInput,
    ...
    lastFileUrl: latestRun?.downloadUrl ?? null,
...
export async function createScheduledExport({ shop, body, subscription }) {
  ...
  const fields = validateFields(body.fields, body.targetGranularity);
  const filename = normalizeFilename(body.filename ?? body.fileName);
  const rawFilterInput = Array.isArray(body.rawFilterInput) ? body.rawFilterInput : [];
  ...
  const targetingPayload = TargetingEngineService.prepareTargetingPayload({
    ...
    legacyFilterParams: rawFilterInput,
...
export async function updateScheduledExport({
  ...
  const fields = body.fields
    ? validateFields(body.fields, body.targetGranularity || existing.targetGranularity)
