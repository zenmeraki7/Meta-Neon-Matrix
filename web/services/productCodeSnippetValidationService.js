...
export function validateProductSnippetDefinition({ title, code }) {
  ...
  if (normalizedTitle.length > 255) {
    throw new Error("Snippet title cannot exceed 255 characters");
  }
  if (sourceCode.length > 262144) {
    throw new Error("Snippet code cannot exceed 262144 characters");
  }

  const ast = parseProductSnippetCode(sourceCode);
