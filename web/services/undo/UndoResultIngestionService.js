...
async function resolveUndoResultUrl({ session, bulkOperationId, resultUrl }) {
  ...
  const response = await client.request(
    ...
    `,
    { variables: { id: String(bulkOperationId) } }
  );
...
async function loadTrustedUndoReplay({ history, undo, shop }) {
  ...
  return { changes, snapshots, snapshotByIdentity };
}
  ...
  if (undo?.undoOperationId) {
    await db.undoOperation.updateMany({
      ...
      data: {
        ...
        errorCode: String(code || "UNDO_FAILED").toUpperCase(),
...
export class UndoResultIngestionService {
  async ingestUndoBulkOperationWebhook({
    ...
    try {
      ...
      if (
        !allowedWebhookTerminalStates.includes(String(undo.state || "")) ||
        ...
        if (undoOperationId) {
          await db.undoOperation.updateMany({
            ...
            data: {
              ...
              errorCode: "UNDO_SHOPIFY_BULK_OPERATION_FAILED",
              errorMessage: `Shopify undo operation ${normalizedStatus}`,
              completedAt: new Date(),
            },
          });
        }
      ...
      if (!verification.verified) {
        ...
        await db.$transaction(
          trustedReplay.snapshots.map((snapshot) =>
            db.targetSnapshotItem.update({
      ...
      if (undoOperationId) {
        await db.undoOperation.updateMany({
          ...
          data: {
            status: isPartial ? "partial" : "completed",
            state: isPartial ? "partially_completed" : "completed",
            bulkOperationId: null,
            processedCount: nextProcessedCount,
