...
test("sync summary DTO remains materially smaller than detail DTO (payload diff guard)", () => {
  const detailLike = {
    ...
    currentProductMirrorBatchId: "batch-1",
    latestSync: {
      id: "sync-1",
  ...
  const summaryLike = {
    ...
    isProductInitiallySyncing: detailLike.isProductInitiallySyncing,
    hasCompletedShopifyBulkJob: detailLike.hasCompletedShopifyBulkJob,
    ...
    currentProductMirrorBatchId: detailLike.currentProductMirrorBatchId,
    latestSync: {
...
test("snapshot-backed history changes project persisted row-level undo evidence", () => {
  ...
  assert.ok(snapshotHistoryBranch.includes("undoMutation: true"));
