...
test("characterization: sync start + status DTOs remain stable", () => {
  ...
  assert.ok(controllerSrc.includes("shopifyBulkOperationId: result.shopifyBulkOperationId"));
  assert.ok(controllerSrc.includes("syncHistoryId: result.syncHistoryId"));
  assert.ok(controllerSrc.includes("mirrorBatchId: result.mirrorBatchId"));
