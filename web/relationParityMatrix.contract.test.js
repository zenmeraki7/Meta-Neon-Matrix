...
function buildContexts() {
  return [
    { targetResourceType: "PRODUCT", shop: "s.myshopify.com", mirrorBatchId: "batch_1" },
    { targetResourceType: "VARIANT", shop: "s.myshopify.com", mirrorBatchId: "batch_1" },
  ];
}
...
test("relation parity matrix compiles deterministically across mixed AST predicates", () => {
  for (const entry of matrixAsts()) {
    for (const context of buildContexts()) {
      ...
      assert.equal(first.whereSql, second.whereSql, `${entry.name}:${context.targetResourceType} whereSql drift`);
      assert.deepEqual(first.params, second.params, `${entry.name}:${context.targetResourceType} params drift`);
