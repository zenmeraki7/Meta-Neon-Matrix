...
export function normalizeCreateAutomaticProductRuleBody(body = {}) {
  ...
  const targetMode = toStringOrNull(src.targetMode || "DYNAMIC_FILTER");
  if (!["DYNAMIC_FILTER", "STATIC_SAVED_SET"].includes(targetMode)) {
    throw createValidationError("Invalid targetMode", "INVALID_TARGET_MODE");
  }
  ...
  const command = {
    ...
    filterParams: Array.isArray(src.filterParams) ? src.filterParams : undefined,
    ...
    scopeType: toStringOrNull(src.scopeType),
