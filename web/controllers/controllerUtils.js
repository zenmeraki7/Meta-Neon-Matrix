...
export async function handleLoggedControllerError({
  ...
  await logApiError({
    ...
    metadata: {
      ...
      resolvedShopId: session?.id || session?.shop || null,
      controller: source,
      query: req?.query || {},
...
function summarizeRequestBody(body) {
  ...
  const filterParams = Array.isArray(body.filterParams) ? body.filterParams : null;
  ...
  return {
    ...
    filterParamsCount: filterParams ? filterParams.length : null,
