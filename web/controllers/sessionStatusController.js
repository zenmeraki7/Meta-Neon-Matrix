...
export async function sessionStatusStreamController(req, res) {
  const shopDomain = String(res.locals.shop || "").trim();
  ...
  try {
    const initial = await getSessionProgress({ shop: shopDomain, sessionId });
    if (!initial) {
      sendEvent(res, "error", { error: "Session not found" });
      cleanup();
      res.end();
    ...
    pollTimer = setInterval(async () => {
      if (closed) return;
      try {
        const session = await getSessionProgress({ shop: shopDomain, sessionId });
        if (!session) {
          sendEvent(res, "error", { error: "Session not found" });
          cleanup();
          res.end();
