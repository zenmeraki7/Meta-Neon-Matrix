...
## CI note: targeting integration tests

Run:

```shell
npm run test:targeting-integration
```

The test file is:

`web/resolveCanonicalProductTarget.integration.test.js`

These tests are intended to run in CI only when both are available:

- `DATABASE_URL` points to a reachable Postgres database
- Prisma client has been generated at `web/generated/prisma/index.js`

If either is missing, the integration tests skip by design.

Normalized targeting can be toggled with:

- `ENABLE_NORMALIZED_TARGETING=true` (default behavior)
- `ENABLE_NORMALIZED_TARGETING=false` (rollback to non-normalized fallback)

Legacy compatibility rollback flags (default retired/off):

- `ENABLE_LEGACY_WHERE_REWRITE=false`
- `ENABLE_LEGACY_NEGATIVE_OPERATORS=false`
- `ENABLE_LEGACY_UNDO_STATUS_FALLBACK=false`
- `ENABLE_LEGACY_RECURRING_FREQUENCY_ALIASES=false`
- `ENABLE_LEGACY_VARIANT_ITEM_FALLBACK=false`

Optional explicit override:

- `NEGATIVE_OPERATOR_MODE=ANTI_JOIN` (allowed: `LEGACY_EXCLUDE`, `NOT_LIKE`, `ANTI_JOIN`)

Ops rollout guidance:

1. Keep all `ENABLE_LEGACY_*` flags set to `false` in normal operation.
2. If a regression is detected, enable only the minimal required flag(s) temporarily.
3. Revert the flag(s) back to `false` after remediation and verification.

For query-plan checks in CI/staging:

```shell
EXPLAIN_SHOP=<shop> EXPLAIN_BATCH=<mirrorBatchId> npm run explain:targeting
```

## Targeting V2 feature flags

- `ENABLE_TARGETING_ENGINE_V2`
  - `false` disables V2.
  - `true` enables V2 for all flows.
  - Comma scopes are also supported: `PREVIEW,EXECUTION,EXPORT,SCHEDULED,RECURRING`.
- `ENABLE_TARGETING_AST_INPUT`
- `ENABLE_TARGETING_LEGACY_ADAPTER_READONLY`
- `ENABLE_TARGETING_STRICT_VALIDATION`
- `ENABLE_TARGETING_PERSIST_VERSIONS`

### Rollout order

1. Shadow mode compare old/new target counts.
2. Enable V2 for preview.
3. Enable V2 for manual execute.
4. Enable V2 for export.
5. Enable V2 for scheduled.
6. Enable V2 for recurring.
7. Disable legacy writes.
8. Keep legacy read adapter.
9. Retire old compilers after a stable window.

Suggested phased value for `ENABLE_TARGETING_ENGINE_V2`:

- `PREVIEW`
- `PREVIEW,EXECUTION`
- `PREVIEW,EXECUTION,EXPORT`
- `PREVIEW,EXECUTION,EXPORT,SCHEDULED`
- `PREVIEW,EXECUTION,EXPORT,SCHEDULED,RECURRING`

## Bulk Edit Preview API risk fields

Endpoint: `POST /api/edit-preview`

The response includes stable risk fields so UI can always show risk before submit:

- `risk.riskLevel`: `LOW | MODERATE | HIGH | CRITICAL`
- `risk.riskScore`: numeric score used for the risk band
- `risk.requiredCriticalConfirmation`: exact typed text required when risk is `CRITICAL` (example: `EDIT 482,391 PRODUCTS`)

These fields are also mirrored under `data` for compatibility:

- `data.riskLevel`
- `data.riskScore`
- `data.requiredCriticalConfirmation`

UI guidance:

1. Show `riskLevel` and `riskScore` directly in preview.
2. If `riskLevel === "CRITICAL"`, require user to type `requiredCriticalConfirmation` exactly before allowing submit.
