 ...
 * @param {{ shopDomain: string, type: "FULL_SYNC"|"INCREMENTAL_SYNC"|"BULK_WRITE", meta?: object }} params
 * @returns {Promise<object>}
 * @throws {Error}
 */
export async function createJob({ shopDomain, type, meta = {} }) {
  ...
  if (!resolvedShopDomain) throw new Error("createJob requires shopDomain");
  if (!resolvedType) throw new Error("createJob requires type");

  const rows = await sql`
    INSERT INTO sync_jobs (shop_id, type, meta)
    VALUES (${resolvedShopDomain}, ${resolvedType}, ${JSON.stringify(resolvedMeta)}::jsonb)
    RETURNING *
  `;
  return rows[0] || null;
}
 ...
 * @throws {Error}
 */
export async function getActiveJob(shopDomain, type) {
  const resolvedShopDomain = String(shopDomain || "").trim();
  const resolvedType = String(type || "").trim();
  if (!resolvedShopDomain) throw new Error("getActiveJob requires shopDomain");
 ...
 * @throws {Error}
 */
export async function getRecentJobs(shopDomain, limit = 10) {
  const resolvedShopDomain = String(shopDomain || "").trim();
  if (!resolvedShopDomain) throw new Error("getRecentJobs requires shopDomain");
