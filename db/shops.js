...
export async function getShop(shopId) {
  ...
  if (!id) throw new Error("getShop requires shopId");
...
export async function deactivateShop(shopId) {
  const id = String(shopId || "").trim();
  if (!id) throw new Error("deactivateShop requires shopId");
 ...
 * @throws {Error}
 */
export async function updateLastSynced(shopId) {
  const id = String(shopId || "").trim();
  if (!id) throw new Error("updateLastSynced requires shopId");
