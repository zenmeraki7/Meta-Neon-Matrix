 ...
 * @throws {Error}
 */
export async function upsertProducts(shopDomain, mirrorBatchId, products) {
  const resolvedShopDomain = String(shopDomain || "").trim();
  const resolvedBatchId = String(mirrorBatchId || "").trim();
  if (!resolvedShopDomain) throw new Error("upsertProducts requires shopDomain");
 ...
 * @throws {Error}
 */
export async function upsertVariants(shopDomain, mirrorBatchId, variants) {
  const resolvedShopDomain = String(shopDomain || "").trim();
  const resolvedBatchId = String(mirrorBatchId || "").trim();
  if (!resolvedShopDomain) throw new Error("upsertVariants requires shopDomain");
 ...
 * @throws {Error}
 */
export async function softDeleteProduct(shopDomain, productId, source) {
  const resolvedShopDomain = String(shopDomain || "").trim();
  const resolvedProductId = BigInt(productId).toString();
  const resolvedSource = String(source || "").trim();
  if (!resolvedShopDomain) throw new Error("softDeleteProduct requires shopDomain");
 ...
 * @throws {Error}
 */
export async function reconcileVariants(shopDomain, productId, liveVariantIds) {
  const resolvedShopDomain = String(shopDomain || "").trim();
  const resolvedProductId = BigInt(productId).toString();
  if (!resolvedShopDomain) throw new Error("reconcileVariants requires shopDomain");
 ...
 * @throws {Error}
 */
export async function getProduct(shopDomain, productId) {
  const resolvedShopDomain = String(shopDomain || "").trim();
  const resolvedProductId = BigInt(productId).toString();
  if (!resolvedShopDomain) throw new Error("getProduct requires shopDomain");
  ...
  const rows = await sql`
    ...
    WHERE p.shop_id = ${resolvedShopDomain}
...
export async function listProducts(
  ...
  const resolvedShopDomain = String(shopDomain || "").trim();
  if (!resolvedShopDomain) throw new Error("listProducts requires shopDomain");
